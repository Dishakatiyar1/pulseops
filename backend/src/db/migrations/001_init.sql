...
-- REQUIRED: BRIN index for time-series range queries
CREATE INDEX idx_checkins_time_brin ON checkins USING BRIN (checked_in);

-- REQUIRED: Composite index for live occupancy query
...
-- Index for gym + time queries (analytics)
CREATE INDEX idx_checkins_gym_time ON checkins (gym_id, checked_in DESC);
...
-- REQUIRED: Composite index for today's revenue query
